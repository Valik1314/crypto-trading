...
static frontend contained in the ``web`` directory so that navigating to
``/`` loads the trading interface.
"""
...
# root of the API, allowing the HTML/CSS/JS frontend to be served without
# requiring an additional web server.
# Compute the absolute path to the web directory relative to this file.  Using
# an absolute path ensures that tests run correctly regardless of the
# current working directory.  ``web`` lives one level above this file
# (crypto_trading/app/main.py → crypto_trading/web).
